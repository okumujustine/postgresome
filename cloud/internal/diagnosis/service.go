...
func (s *Service) RunForDatabase(ctx context.Context, databaseInstanceID string) (*RunResult, error) {
	...
	activitySnapshot, err := repository.ListActivityStats(ctx, s.pool, databaseInstanceID)
	if err != nil {
		return nil, err
	}
	...
	tableSnapshot, err := repository.ListTableStats(ctx, s.pool, databaseInstanceID)
	if err != nil {
		return nil, err
	}
	...
	querySnapshot, err := repository.ListQueryStats(ctx, s.pool, databaseInstanceID)
	if err != nil {
		return nil, err
	}
	...
	explainSnapshot, err := repository.ListExplainPlans(ctx, s.pool, databaseInstanceID)
	if err != nil {
		return nil, err
	}

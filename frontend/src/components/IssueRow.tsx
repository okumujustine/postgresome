...
export function IssueRow({ finding }: { finding: DashboardFinding }) {
  return (
    <div className="flex flex-col gap-3 py-4">
        ...
        <div className="min-w-0">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            {finding.status === 'resolved' && (
              <span
                className="inline-flex h-[20px] shrink-0 items-center rounded-[var(--radius-pill)] px-[8px] text-[11px] font-medium"
                style={{ background: 'var(--success-tint)', color: 'var(--success)', border: '1px solid rgba(26,127,55,0.18)' }}
              >
                Resolved
              </span>
            )}
            <span className="text-[12px]" style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>
      ...
      <div className="grid gap-3 text-[13px] md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div className="min-w-0">
          <div className="mb-1 text-[12px] font-medium" style={{ color: 'var(--text-muted)' }}>
            Problem
          </div>
          <div className="leading-[1.5]" style={{ color: 'var(--text-secondary)' }}>
            {finding.message}
          </div>
        </div>
        <div className="min-w-0">
